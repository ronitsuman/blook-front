...
export default function PaymentPage() {
  ...
  useEffect(() => {
    const paymentData = JSON.parse(localStorage.getItem("blook_booking_payment"));
    if (!paymentData) {
      navigate("/dashboard/advertiser");
      return;
    }
    // Razorpay script load ho chuka hona chahiye!
    const options = {
      key: paymentData.key,
      ...
      handler: async function (response) {
        // Razorpay payment success, ab verify API call karo
        try {
          ...
          await axiosInstance.post("/payments/verify", {
            ...
            bookingId: paymentData.bookingId, // your own storage
          });
          
          alert("Payment Success! Booking updated.");
        } catch {
          alert("Payment done but could not verify, contact support.");
        }
        navigate("/dashboard/advertiser");
      },
      theme: { color: "#4338ca" }
    ...
    const rzp = new window.Razorpay(options);
    rzp.open();
    return () => localStorage.removeItem("blook_booking_payment");
  }, [navigate, orderId]);

  return (
    <div className="p-16 text-center">Opening payment gateway...</div>
  );
}
